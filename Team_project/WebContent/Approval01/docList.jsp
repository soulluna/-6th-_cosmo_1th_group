...
<script>
	...
	function datepickerValueReset() {
		document.getElementsByName('datepicker1')[0].value = "";
		document.getElementsByName('datepicker2')[0].value = "";
		frm.submit();
	}
</script>
		...
		<div class="deptContent" style="margin: 0 10px 10px 10px;">
			...
			<form name="frm" method="get">
				<div class="serachArea" style="margin-left:51px;">
					...
					<%-- <input type="button" onclick="location.href = '${contextPath}/Approval/docList.do'" value="모든 검색 조건 초기화" style="width: 180px; position: absolute; top:615px; left: 50px;"> --%>
					<span>
					...
					<span style="margin-left:30px;">
						<input type="text" id="datepicker1" name="datepicker1" placeholder="년/월/일" readonly value="${datepicker1}">
						~
						<input type="text" id="datepicker2" name="datepicker2" placeholder="년/월/일" readonly value="${datepicker2}">
				...
				<div class="side">
					<a
						href="${contextPath}/Approval/docList.do?sendReceive=수신&serachDocState=${serachDocState}&serachDocList=${serachDocList}&searchType=${searchType}&searchKey=${searchKey}&sendReceive=${sendReceive}&datepicker1=${datepicker1}&datepicker2=${datepicker2}"
					>
						<div>수신</div>
					</a> <a
						href="${contextPath}/Approval/docList.do?sendReceive=발신&serachDocState=${serachDocState}&serachDocList=${serachDocList}&searchType=${searchType}&searchKey=${searchKey}&sendReceive=${sendReceive}&datepicker1=${datepicker1}&datepicker2=${datepicker2}"
					>
						<div>발신</div>
					</a>
				</div>
					...
					<c:forEach var="page" begin="${(pagingMap.pageSessionNum-1)*5+1}" end="${pagingMap.pageSessionNum*5}">
						<c:choose>
							<c:when test="${pagingMap.pageNum == page}">
								<a
									href="${contextPath}/Approval/docList.do?pageNum=${page}&pageSession=${pagingMap.pageSessionNum}&sendReceive=${sendReceive}&serachDocState=${serachDocState}&serachDocList=${serachDocList}&searchType=${searchType}&searchKey=${searchKey}&sendReceive=${sendReceive}&datepicker1=${datepicker1}&datepicker2=${datepicker2}"
									style="color: red"
								>${page}</a>
							</c:when>
